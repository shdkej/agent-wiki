...
const mappedRoot = path.join(docsRoot, 'outputs');
const sourceRoot = process.env.MANDALART_SOURCE_ROOT ?? path.resolve(repoRoot, '../source/shdkej-content');
const manifestPath = path.join(docsRoot, 'mandalart-core-inventory.json');
...
export function verify() {
  ...
  if (!linkedTargets(index).includes('outputs/source-category-map')) throw new Error('index.mdx must link to outputs/source-category-map');
