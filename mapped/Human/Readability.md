...
## 이번 갱신 메모
- 기존 baseline은 유지한 채 `언어 해독 가능성`, `전문어와 일상어의 경계`, `쉬운 설명과 공부의 거리`, `쉬운 길 vs 쉬운 일`을 더 세밀하게 분리했다.
- 원문 곳곳의 예시를 `읽는 상황`, `의미 전달`, `학습 태도`라는 층위로 다시 묶어 가독성을 문장 기술보다 전달 인터페이스 문제로 읽히게 다듬었다.
- 바로 재사용 가능한 질문 묶음을 추가해, 이후 이 노드를 글쓰기뿐 아니라 학습·설명·코드 커뮤니케이션 판단에도 다시 쓰기 쉽게 만들었다.
