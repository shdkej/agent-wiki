...
## 원문 발췌 구조
원문은 아래 순서로 짧고 강하게 배치되어 있다.

1. `여유, 관심, 사랑, 유머, 용기, 표현력, 솔직함, 적극성`처럼 먼저 사람이 어떤 상태가 되어야 하는지 적는다.
2. `몰입, 편집, 정리, 밸런스, 간결, 변증법, 전체, 하루, 피드백, 순환, 유연함, 관찰`처럼 좋아하는 작업 문법을 적는다.
3. 행동방침 표에서 `꿈꾸는 영역`, `하루 회고`, `생산자`, `시작점과 끝점`, `마이크로어드벤처`, `페인포인트`, `주도성`을 짧은 선언으로 건다.
4. `해야한다` 목록에서 호기심, 대화 연습, 여유시간, 작은 전진, 통제 가능한 범위, 분류-아웃풋-피드백, 활용까지의 실천 규칙을 붙인다.
5. 마지막에 `나`, `세상에서 효과적인 도구`로 자기 성향과 반복 사용할 사고 도구를 메모한다.

이 배치 덕분에 이 노드는 명언 수집이 아니라 `상태 → 작업 문법 → 행동 규칙 → 자기 인식 → 도구`의 순서로 스스로를 조율하는 카테고리로 읽힌다.
...
여기서 중요한 것은 `많이 쌓는다`와 `선택과 집중`이 서로 모순되지 않는다는 점이다. 먼저 넓게 모으되, 나중에는 분류와 편집으로 핵심을 뽑아 하나의 결과물로 만든다. 즉 이 노드의 생산성은 처음부터 적게 가지는 금욕보다, **많이 보고 다시 덜어내는 편집 능력**에 더 가깝다.
...
특히 `헬스할때도 독서모임할때도 역사모임할때도 회사에서도 친구들과도`라는 식의 나열은, 관계 훈련을 특별한 이벤트가 아니라 생활 전반의 반복 훈련으로 본다는 뜻에 가깝다. 말하기는 성격 문제가 아니라 장면마다 다시 호출해야 하는 실전 능력으로 적혀 있다.
...
## Principle과 Standard의 차이
[[../../mapped/Human/Standard]]도 기준과 제한을 다루지만, 두 노드의 초점은 다르다.

- `Principle`은 **나는 어떤 상태와 태도로 살 것인가**를 먼저 묻는다.
- `Standard`는 **기준·룰·템플릿을 어떻게 만들고 언제 깨야 하는가**를 더 많이 묻는다.

둘은 연결되지만 역할은 다르다. `Principle`이 자기 내부의 방향타라면, `Standard`는 바깥 구조를 설계하거나 의심할 때 쓰는 판단 도구에 가깝다. 그래서 `Principle`을 읽을 때는 제도 설계보다 생활 리듬, 관계 태도, 생산자 감각을 먼저 봐야 드리프트가 줄어든다.

## 이번 심화에서 더 선명해진 점
- 영어 중심의 기존 설명을 한국어 기준으로 다시 묶어, 원문의 정서와 생활 감각이 더 직접 드러나게 했다.
- `원문 발췌 구조`를 추가해 실제 원문의 배치 순서가 `상태 → 작업 문법 → 행동 규칙 → 자기 인식`으로 이어진다는 점을 바로 보이게 했다.
...
- [[../../mapped/Human/Standard]]와의 차이를 분리해, 이 페이지는 룰 설계보다 자기 삶의 기본 태도와 작업 문법을 세우는 쪽에 더 가깝다는 점을 보강했다.
