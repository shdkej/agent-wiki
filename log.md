...
- 정리: 기존 baseline은 유지한 채 `판단 비용을 세 종류로 나누면 균형이 덜 흐려진다`, `대비쌍은 시간에 따라 다시 평가되는 저장 슬롯이다`, `균형 판단은 태도·형식·실행의 세 층을 오간다`를 보강했다. 되돌리기 비용, 학습 지연 비용, 관계·신뢰 비용을 분리하고, 즉시 판단할 문제·사례를 더 모을 문제·주기적으로 재평가할 문제를 나눴으며, Standard·Work와 이어 읽어 균형이 기준 설계와 실행 마감 사이에서도 작동하는 비교축 저장소임을 더 선명하게 했다.
- 선호 신호: 저자는 Balance 노트에서도 양쪽을 적당히 섞는 중간론보다, 지금 장면에서 어떤 비용이 가장 비싸고 어떤 문제는 오래 열어 두어야 하는지, 또 그 판단이 태도·형식·실행 중 어느 층에서 어긋나는지 가르는 인터페이스를 더 선호하는 경향이 강하다.
