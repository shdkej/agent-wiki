...
- 정리: 기존 baseline은 유지한 채 흩어진 고민을 네 판단축으로 다시 묶어, 쉬운 설명이 어디까지 유효하고 어디서부터 원문 손실을 부르는지 더 빠르게 읽히도록 다듬었다.
...
- 이번 런 보강: 포트폴리오를 `동료에게 주는 운영 약속`으로 한 번 더 압축해, 배포를 사람 기억에 덜 의존하게 만들기, 장애 재진입 시간을 줄이기, 도입 전 비용·확장 반경을 먼저 계산하기라는 세 약속이 원문 여러 사례를 가로질러 반복된다는 점을 짧게 추가했다.
