...
## 2026-07-08 10:00 UTC | Marketing 아하/활성화 분리 승격

- 갱신 범위: `source/external-links/marketing/2026-07-02-aha-vs-activation-split.md` 1개 외부 링크를 승격 대상으로 처리하고, [[mapped/Integration/Marketing]]의 `첫 가치`/`결과 소유 행동`/`첫 사용자 학습` 관찰 경계에 흡수했다.
- 탐구: summarized 마케팅 백로그가 80개로 30개를 초과해 새 학습 노트를 만들지 않고 기존 백로그를 앞으로 이동했다.
- 정리: 첫 세션의 아하 경험과 내부 activation 이벤트를 한 숫자로 합치지 않고, J3의 `judged but not saved`처럼 자연 종료일 수 있는 상태를 자동 실패로 읽지 않는 기준으로 압축했다.
- 선호 신호: Virtue prelaunch에서는 이벤트 완료 여부보다 사용자가 어떤 순간을 가치로 느꼈는지와 그 언어가 다음 카피·온보딩 판단에 재사용되는지가 더 중요하다.
