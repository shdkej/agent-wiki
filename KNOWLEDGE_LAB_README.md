...
- 공개 Agent Wiki의 검색·`llms.txt`·문서 URL은 컴파일된 `agent-wiki/content/docs/`만 조회한다. raw source와 `queued` 자료는 공개 경로로 노출하지 않는다.
- 원자료 확인은 로컬 전용 `python3 ingest/query_raw.py "검색어"`로 수행한다. 기본 검색 범위는 사람이 검토한 `reviewed`·`selected`·`integrated` 상태이며, `queued`는 `--include-queued`를 명시한 내부 검토에서만 조회한다.
- source 발견과 manifest 갱신은 `scripts/sync-ingest.sh`가 담당하고, 의미 판정·위키 컴파일은 별도 작업으로 남긴다. 세부 SLA와 상태 전이는 `ingest/POLICY.md`가 정본이다.
